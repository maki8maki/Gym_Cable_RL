...
if __name__ == '__main__':
    ...
    gathering_data = True
    data_path = f'./data/buffer_o-3_w-hs_{action_space.shape[0]}_{memory_size}.pcl'
    if gathering_data:
    ...
    for episode in tqdm(range(nepisodes)):
        ...
        if (episode+1) % (nepisodes/100) == 0:
            ...
            agent.train()
    anim(frames, titles=titles, filename="out/test_rl_o-3_w-trainedAE_xyz-1.mp4", show=False)
    ...
    anim(frames, titles=titles, filename="out/test_rl_o-3_w-trainedAE_xyz-2.mp4", show=False)
    agent.save("model/test_rl_o-3_w-trainedAE_xyz.pth")
